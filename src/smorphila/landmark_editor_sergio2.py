...
def deserialize_json(
    ...
    axis_data = project_definition.get("main_axis")
    main_axis = None
    warnings = []
    if axis_data is not None:
        if not isinstance(axis_data, dict):
            raise ValueError("main_axis must be an object")
...
class AxisAlignmentDialog(QDialog):
    """Choose how the antero-posterior axis is aligned on screen."""

    def __init__(self, parent: QWidget | None = None):
        super().__init__(parent)
        self.alignment = "vertical"
        self.setWindowTitle("Set antero-posterior axis alignment")
        layout = QVBoxLayout(self)
        instruction = QLabel(
            "Choose how the image should be rotated to align the axis."
        )
        instruction.setWordWrap(True)
        layout.addWidget(instruction)
...
class ImageCanvas(QWidget):
    ...
    def _draw_pending_axis_origin(self, painter: QPainter):
        ...
        painter.setBrush(QColor("#ffffff"))
        painter.drawEllipse(origin, 6, 6)
        painter.drawText(origin + QPointF(8, -8), "Selected axis landmark")
...
class LandmarkEditor(QMainWindow):
    ...
    def _build_group_section(self) -> QGroupBox:
        ...
        layout = QVBoxLayout(group)
        description = QLabel(
            "Define the antero-posterior axis first, then add groups and segments."
        )
        description.setWordWrap(True)
        layout.addWidget(description)
        axis_buttons = QHBoxLayout()
        self.define_axis_button = QPushButton("Define antero-posterior axis")
        self.clear_axis_button = QPushButton("Clear axis")
        axis_buttons.addWidget(self.define_axis_button)
        axis_buttons.addWidget(self.clear_axis_button)
    ...
    def _start_axis_definition(self):
        ...
        self.pending_axis_points.clear()
        self.pending_axis_landmark = None
        self.status_bar.showMessage("Click the first antero-posterior axis landmark.")
        self.canvas.update()
    ...
    def _clear_axis(self):
        ...
        self.canvas.update()
        self.status_bar.showMessage("Antero-posterior axis cleared.")
    ...
    def _delete_group(self):
        ...
        if name is None:
            QMessageBox.warning(self, "Group", "The antero-posterior axis cannot be deleted here.")
            return
    ...
    def _handle_canvas_click(self, image_point: QPointF):
        ...
        if self.mode == "axis":
            ...
            if not self.pending_axis_points:
                ...
                self.status_bar.showMessage("Click the other antero-posterior axis landmark.")
                self.canvas.update()
                return
            origin = self.pending_axis_points[0]
            if self.pending_axis_landmark == landmark_name:
                QMessageBox.warning(
                    self, "Antero-posterior axis", "The two landmarks must be distinct."
                )
                return
            ...
            self._refresh_groups()
            self.canvas.update()
            self.status_bar.showMessage("Antero-posterior axis aligned.")
            return
    ...
    def _refresh_groups(self):
        ...
        axis_state = "defined" if self.main_axis is not None else "not defined"
        self.group_list.addItem(f"Antero-posterior axis ({axis_state})")
        self.group_list.item(0).setData(Qt.UserRole, "__main_axis__")
        for group in self.groups.values():
